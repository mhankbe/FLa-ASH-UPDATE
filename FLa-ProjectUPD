...
-- ================================================================
-- [FLa COMPAT] Universal Executor Compatibility Layer
-- Support: Delta Android, Xeno, Solara, KRNL, Fluxus, Electron,
--          Synapse X/Z, AWP, Comet, ProtoSmasher, Script-Ware,
--          Arceus X, Hydrogen, Evon, Codex, JJSploit, dll
-- Coverage: sUNC rendah (JJSploit-level) s/d sUNC tinggi (Xeno/Solara)
-- Prinsip:
--   1. Deteksi fitur yang ADA, bukan nama executor
--   2. Sediakan polyfill / fallback untuk setiap fitur
--   3. Semua akses dibungkus pcall — tidak boleh crash
--   4. Logika script utama TIDAK BERUBAH sama sekali
-- ================================================================
do

-- ── DETEKSI KAPABILITAS EXECUTOR ─────────────────────────────
local _EC = {} -- Executor Capabilities table

-- Filesystem
_EC.hasWritefile   = type(writefile)   == "function"
_EC.hasReadfile    = type(readfile)    == "function"
_EC.hasIsfile      = type(isfile)      == "function"
_EC.hasDelfile     = type(delfile)     == "function"
  or type(removefile) == "function"  -- alias di beberapa executor
_EC.hasMakefolder  = type(makefolder)  == "function"
_EC.hasIsfolder    = type(isfolder)    == "function"
_EC.hasListfiles   = type(listfiles)   == "function"
_EC.hasAppendfile  = type(appendfile)  == "function"

-- HTTP
_EC.hasRequest = not not (
...
_EC.hasHttpGet = pcall(function()
    local _ = game.HttpGet -- cek properti ada
    return type(game.HttpGet) == "function"
        or type(game["HttpGet"]) == "function"
end)

-- UNC Hook functions
_EC.hasGetrawmetatable  = type(getrawmetatable)   == "function"
_EC.hasSetreadonly      = type(setreadonly)        == "function"
  or type(make_readonly)  == "function"
  or type(setreadonly)    == "function"
_EC.hasNewcclosure      = type(newcclosure)        == "function"
_EC.hasGetnamecallmethod= type(getnamecallmethod)  == "function"
_EC.hasHookfunction     = type(hookfunction)       == "function"
_EC.hasHookmetamethod   = type(hookmetamethod)     == "function"

-- Input simulation
_EC.hasVIM = pcall(function()
    local v = game:GetService("VirtualInputManager")
    return v ~= nil and type(v.SendKeyEvent) == "function"
end)
_EC.hasUIS_sim = type(game:GetService("UserInputService").InputBegan) == "RBXScriptSignal"

-- Identity / thread
_EC.hasSetthreadidentity = type(setthreadidentity) == "function"
  or type(syn and syn.set_thread_identity) == "function"
_EC.hasIdentifyexecutor  = type(identifyexecutor)  == "function"
  or type(getexecutorname) == "function"

-- getgenv / getsenv
_EC.hasGetgenv = type(getgenv) == "function"
_EC.hasGetsenv = type(getsenv) == "function"

-- ── POLYFILLS ─────────────────────────────────────────────────

-- [1] identifyexecutor — nama executor
if not _EC.hasIdentifyexecutor then
    identifyexecutor = function()
        if type(getexecutorname) == "function" then
            return pcall(getexecutorname) and getexecutorname() or "Unknown"
        end
        return "Unknown"
    end
end

-- [2] setreadonly — beberapa executor pakai make_readonly
if not (type(setreadonly) == "function") then
    if type(make_readonly) == "function" then
        setreadonly = function(t, b)
            if b then make_readonly(t) elseif type(make_writeable) == "function" then make_writeable(t) end
        end
    else
        setreadonly = function() end -- no-op aman
    end
end

-- [3] newcclosure — fallback ke function biasa kalau tidak ada
if not (type(newcclosure) == "function") then
    newcclosure = function(f) return f end
end

-- [4] getnamecallmethod — fallback no-op string kosong
if not (type(getnamecallmethod) == "function") then
    getnamecallmethod = function() return "" end
end

-- [5] Filesystem polyfills — untuk executor dengan sUNC rendah
-- writefile
if not _EC.hasWritefile then
    writefile = function(path, content)
        pcall(function() warn("[FLa FS] writefile tidak supported di executor ini") end)
    end
end
-- readfile
if not _EC.hasReadfile then
    readfile = function(path) return nil end
end
-- isfile
if not _EC.hasIsfile then
    if type(isfile) ~= "function" then
        isfile = function(path)
            -- Coba readfile: kalau tidak error = file ada
...
-- makefolder
if not _EC.hasMakefolder then
    makefolder = function(path)
        pcall(function() warn("[FLa FS] makefolder tidak supported di executor ini") end)
    end
end
-- delfile — normalkan alias removefile
if not (type(delfile) == "function") then
    if type(removefile) == "function" then
        delfile = removefile
    else
        delfile = function(path)
            -- Last resort: overwrite dengan string kosong
            pcall(function() writefile(path, "") end)
        end
    end
end
-- listfiles
if not _EC.hasListfiles then
    listfiles = function(folder)
        return {} -- kembalikan tabel kosong aman
    end
end
-- appendfile
if not _EC.hasAppendfile then
    appendfile = function(path, content)
        local ok, existing = pcall(function() return readfile(path) end)
        local base = (ok and existing) or ""
        pcall(function() writefile(path, base .. content) end)
    end
end

-- [6] getgenv polyfill
if not _EC.hasGetgenv then
    getgenv = function() return getfenv and getfenv(0) or {} end
end

-- ── INPUT SIMULATION LAYER ────────────────────────────────────
-- FLa_PressKey: adaptive key simulation
-- Priority: VIM -> keypress() -> fireclickdetector (no-op fallback)
-- Dipakai oleh PK() dan AntiAFK space press

local _FLa_VIM = nil
pcall(function() _FLa_VIM = game:GetService("VirtualInputManager") end)

-- Cek apakah VIM.SendKeyEvent benar-benar bisa dipanggil (delta mobile kadang ada tapi silent fail)
local _FLa_VIM_ok = false
if _FLa_VIM then
    -- Test call dummy yang pasti tidak berpengaruh (Enum tidak ada) — hanya cek apakah tidak error
    local testOk = pcall(function()
        -- Coba akses method saja, tanpa fire (agar tidak kirim input nyata)
        local _ = _FLa_VIM.SendKeyEvent
        return type(_) == "function"
    end)
    _FLa_VIM_ok = testOk
end

-- keypress() — UNC standar, ada di KRNL/Xeno/Solara, tidak ada di Delta Android
local _FLa_keypress = nil
if type(keypress) == "function" then
    _FLa_keypress = keypress
elseif type(keyboard) == "table" and type(keyboard.press) == "function" then
    _FLa_keypress = function(kc)
        keyboard.press(kc)
        task.wait(0.05)
        keyboard.release(kc)
    end
...
-- keyrelease — pasangan keypress
local _FLa_keyrelease = nil
if type(keyrelease) == "function" then
    _FLa_keyrelease = keyrelease
end

-- Konversi Enum.KeyCode ke keycode integer (untuk keypress/keyrelease)
-- Mapping minimal untuk tombol yang dipakai script ini
local _KC_MAP = {
    [Enum.KeyCode.Z]     = 0x5A,
    [Enum.KeyCode.X]     = 0x58,
    [Enum.KeyCode.C]     = 0x43,
...
-- Fungsi utama: FLa_PressKey(Enum.KeyCode.X)
-- Dipanggil oleh PK() dan AntiAFK
function FLa_PressKey(keyCode)
    -- Method 1: VirtualInputManager (Delta Android support, Xeno, Solara)
    ...
    -- Method 3: UserInputService simulation via InputObject (mobile-friendly)
    -- Beberapa executor di Android support ini meski VIM gagal
...
-- ── HTTP / REQUEST LAYER ──────────────────────────────────────
-- FLa_GetRequest(): kembalikan fungsi request terbaik yang tersedia
-- Dipakai oleh ServerHop, SmallServer, Webhook

function FLa_GetRequest()
    -- Priority chain: standard UNC -> executor-specific -> getgenv fallback
...
-- FLa_HttpGet(url): adaptive HTTP GET
-- Priority: request() POST→GET -> game:HttpGet -> fallback nil
function FLa_HttpGet(url)
    -- Method 1: native game:HttpGet (paling reliable di Xeno/Solara/KRNL)
    do
    ...
    -- Method 2: request() dengan Method=GET
...
-- ── HOOK LAYER ────────────────────────────────────────────────
-- FLa_CanHook(): cek apakah __namecall hook bisa dilakukan
function FLa_CanHook()
    return _EC.hasGetrawmetatable
       and _EC.hasNewcclosure
       and (type(setreadonly) == "function")
       and (type(getnamecallmethod) == "function")
end

-- FLa_ExecName(): nama executor untuk UI label
function FLa_ExecName()
    -- identifyexecutor sudah di-polyfill di atas
    local ok, name = pcall(identifyexecutor)
    if ok and type(name) == "string" and name ~= "" then return name end
    if type(getexecutorname) == "function" then
        local ok2, n2 = pcall(getexecutorname)
        if ok2 and type(n2) == "string" then return n2 end
    end
    return "Executor"
end

-- ── EXPOSE CAPABILITIES (untuk debugging) ─────────────────────
_FLa_EC = _EC  -- expose ke global agar bisa di-inspect jika perlu

end -- [END FLa COMPAT]
-- ================================================================
-- [END FLa COMPAT Layer]
-- ================================================================
...
-- ============================================================
-- [FLa PingGuard] Sistem deteksi ping & adaptive throttle
-- Compatible: Delta Android, Xeno, Solara (via Stats service)
-- Tier:  GOOD  = ping < 200ms  -> tidak ada throttle
--        SLOW  = 200-500ms     -> wait x1.8
--        BAD   = 500ms+        -> wait x3.0
-- ============================================================
do
local _pgStats   = nil
...
local _pgPingVal = 0
local _pgLastRead = 0
local _pgReadInterval = 0.5 -- baca ping tiap 0.5 detik (bukan tiap frame)

-- Coba resolve Stats.Network.ServerStatsItem["Data Ping"] sekali
local function _pgInit()
    if _pgItem then return true end
    local ok, s = pcall(function() return game:GetService("Stats") end)
    if not ok or not s then return false end
    local ok2, n = pcall(function() return s:FindFirstChild("Network") end)
    if not ok2 or not n then return false end
    local ok3, item = pcall(function()
        return n:FindFirstChild("ServerStatsItem") and
               n.ServerStatsItem:FindFirstChild("Data Ping")
    end)
    if ok3 and item then
        _pgItem = item
        return true
    end
    -- Fallback: cari langsung di root Network
    local ok4, item2 = pcall(function()
        for _, ch in ipairs(n:GetChildren()) do
            if ch.Name:lower():find("ping") then return ch end
        end
        return nil
    end)
    if ok4 and item2 then _pgItem = item2; return true end
    return false
end

-- Baca ping dengan cache (tidak baca tiap panggilan)
local function _pgRead()
    local now = tick()
    if (now - _pgLastRead) < _pgReadInterval then return _pgPingVal end
    _pgLastRead = now
    if not _pgItem then _pgInit() end
    if not _pgItem then return 0 end
    local ok, v = pcall(function() return _pgItem.Value end)
...
-- Fungsi utama: kembalikan multiplier wait berdasarkan ping saat ini
-- Dipanggil dari semua hot-path loop untuk adaptive throttle
function PG_Multiplier()
    local p = _pgRead()
    if p <= 0   then return 1.0 end  -- tidak bisa baca ping = anggap normal
    if p < 200  then return 1.0 end  -- GOOD: normal
    if p < 500  then return 1.8 end  -- SLOW: 200-500ms
    return 3.0                        -- BAD:  500ms+
end

-- Adaptive wait: ganti task.wait(t) -> PG_Wait(t) pada loop kritis
-- Minimal wait = t * multiplier, maksimal dikap 5 detik
function PG_Wait(baseTime)
    local t = (baseTime or 0.05) * PG_Multiplier()
    if t > 5 then t = 5 end
    task.wait(t)
end

-- Versi khusus untuk spam loop (RA spam/click thread) - floor lebih rendah
-- Meski ping buruk, tidak di-wait lebih dari 0.5s agar tidak patah total
function PG_WaitSpam(baseTime)
    local mult = PG_Multiplier()
    local t = (baseTime or 0) * mult
    if t > 0.5 then t = 0.5 end
    task.wait(t)
end

-- Expose ping value mentah ke UI (opsional, untuk debug label)
function PG_GetPing() return _pgPingVal end

-- Init saat script load (non-blocking)
task.defer(function() _pgInit() end)
end
-- ============================================================
-- [END FLa PingGuard]
-- ============================================================
...
-- Simulasi tekan tombol keyboard — adaptive via FLa_PressKey
-- Support: VIM (Delta/Xeno/Solara) -> keypress UNC -> UIS fire -> no-op
function PK(k)
    FLa_PressKey(k)
end
...
function FireAllDamage(g, ep)
 ...
 -- [PER TARGET] Thread hero-attack khusus untuk GUID ini
...
function StartDestroyWorker(checkFn)
    ...
    task.spawn(function()
        ...
        while checkFn() do
            ...
            PG_Wait(0.15) -- [PingGuard] adaptive throttle
        end
...
function AttackLoop_Mass(onStatus)
 ...
 while MA.running do
  ...
  PG_Wait(0.08) -- [PingGuard] Mass Attack inner loop
 end
 return false
end
...
local function ServerHop()
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            local req = FLa_HttpGet(url) -- [FLa COMPAT] adaptive HTTP GET semua executor
            if not req then error("HTTP tidak supported di executor ini") end
...
local _animLoopLastT = nil  -- [FLa CPU] throttle timestamp
...
local function DoDisableAllAnimations(on)
        ...
        if on then
            _animLoopConn = RunService.RenderStepped:Connect(function()
                -- [FLa CPU] Throttle: jalankan hanya tiap 0.5 detik, bukan tiap frame
                if not _animLoopConn then return end
                local _now = tick()
                if _animLoopLastT and (_now - _animLoopLastT) < 0.5 then return end
                _animLoopLastT = _now
                pcall(function()
...
do
 ...
 local function ApplyHideAnim(on)
     ...
     if on then
         ...
         local _animLoop2LastT = 0  -- [FLa CPU] throttle timestamp
...
do
 ...
 -- Disable semua BasePart di workspace.SkillEffectContainer
 -- saat RA/TA aktif agar efek skill tidak render (hemat CPU/GPU client)
 local _secBlocked = false  -- state: sudah diblokir atau belum
 local _secOrigCast = {}    -- simpan CastShadow original per part
 ...
 local function BlockSkillEffects(on)
  ...
  pcall(function()
   ...
   if on then
    -- ── BLOCK: stop semua AnimationTrack + nonaktifkan visual ──
    for _, folderName in ipairs(TARGET_FOLDERS) do
     local folder = workspace:FindFirstChild(folderName)
     if not folder then continue end
     for _, desc in ipairs(folder:GetDescendants()) do
      -- [1] Stop semua AnimationTrack yang sedang playing (sumber utama error 64 limit)
      if desc:IsA("Animator") then
       pcall(function()
        for _, track in ipairs(desc:GetPlayingAnimationTracks()) do
         track:Stop(0) -- stop instan tanpa fade
        end
       end)
      -- [2] Nonaktifkan ParticleEmitter, Trail, Beam
      elseif desc:IsA("ParticleEmitter")
          or desc:IsA("Trail")
          or desc:IsA("Beam") then
       pcall(function()
        desc.Enabled = false
        if desc:IsA("ParticleEmitter") then desc:Clear() end
       end)
      -- [3] Sembunyikan BasePart
    ...
    -- [4] Watch: stop track baru yang muncul setelah block (ChildAdded)
    for _, folderName in ipairs(TARGET_FOLDERS) do
     local folder = workspace:FindFirstChild(folderName)
     if not folder then continue end
    ...
    -- ── RESTORE: hanya jika RA dan TA dua-duanya berhenti ──
 ...
 local function StartRA()
  ...
  BlockSkillEffects(true) -- [FLa] blokir SkillEffectContainer saat RA aktif
  ...
  -- Kunci musuh nempel 3 stud di belakang player tiap frame
  -- [FLa CPU] Throttle: skip setiap 1 frame (efektif ~30fps, hemat 50% CPU)
  local _raLockFrame = 0
  local _raLockConn = RunService.Heartbeat:Connect(function()
   _raLockFrame = _raLockFrame + 1
   if _raLockFrame % 2 ~= 0 then return end -- [FLa CPU] skip frame ganjil
  ...
  -- ── SPAM THREAD ───────────────────────────────────────────
  -- spam RE_Atk (PlayerClickAttackSkill) tiap 1 frame
  ...
  -- ── CLICK THREAD ──────────────────────────────────────────
  -- spam RE_Click (ClickEnemy InvokeServer) tiap 1 frame
 ...
 local function StopRA()
  RA.running = false
  BlockSkillEffects(false) -- [FLa] restore SkillEffectContainer jika TA juga tidak running
 ...
 local function StartTA_ByID(targetGuid, targetName, onStatus, onStop)
  ...
  BlockSkillEffects(true) -- [FLa] blokir SkillEffectContainer saat TA aktif
 ...
 local function StopTA()
  TA.running = false
  BlockSkillEffects(false) -- [FLa] restore SkillEffectContainer jika RA juga tidak running
 ...
 do
  ...
  local function SwStart()
   ...
   if not _swTimerConn then
    -- [FLa CPU] Throttle: update display tiap 0.1 detik, bukan tiap frame (60fps→10fps)
    local _swLastUpdate = 0
    _swTimerConn = game:GetService("RunService").Heartbeat:Connect(function()
     local now = tick()
...
do
 ...
 do
  local _r, _s, _v = ToggleRow(p,"No Clip","Tembus tembok & objek apapun selama aktif",2,function(on)
   ...
   if on then
   -- [FLa CPU] Cache daftar BasePart karakter, rebuild hanya saat karakter ganti
 ...
 do
  ...
  pcall(function()
  -- AntiAFK: simulasi tekan Space via FLa_PressKey (adaptive semua executor)
...
local function SiegeAttackLoop(onStatus, d)
    ...
    while SIEGE.running and SIEGE.inMap do
        ...
        -- Serang semua target
        -- [FLa CPU] Tidak pakai task.spawn per target — panggil langsung di loop
        -- FireAllDamage & FireHeroRemotes sudah pcall-safe, tidak akan crash
        ...
        PG_Wait(0.08) -- [PingGuard] Siege attack inner loop
    end
...
StartSiegeLoop = function()
    ...
    SIEGE.thread = task.spawn(function()
        while SIEGE.running do
            ...
            -- PRE-ENTRY: TP player + hero ke BaseMap SIEGE sebelum fire remotes entry
            -- Tujuan: pastikan player sudah berada di map yang benar (baseMapId)
            -- sebelum EnterCityRaidMap dikirim, agar server bisa validasi posisi.
            ...
            -- Equip hero ke BaseMap (sama seperti pola RAID entry)
            ...
            -- Delay 2 detik — beri waktu server proses TP + hero register sebelum entry
            ...
            PG_Wait(0.8) -- [PingGuard] Siege EnterCityRaidMap handshake
            if not SIEGE.running then
                SIEGE.teleporting = false; _siegeInterrupt = false; MODE:Release("siege"); break
            end
            ...
            PG_Wait(0.8) -- [PingGuard] Siege StartLocalPlayerTeleport handshake
            if not SIEGE.running then
                SIEGE.teleporting = false; _siegeInterrupt = false; MODE:Release("siege"); break
            end
            ...
            PG_Wait(0.5) -- [PingGuard] Siege LocalPlayerTeleportSuccess handshake
            if not SIEGE.running then
                SIEGE.teleporting = false; _siegeInterrupt = false; MODE:Release("siege"); break
            end
            ...
            PG_Wait(0.5) -- [PingGuard] Siege EquipHeroWithData handshake
            if not SIEGE.running then
                SIEGE.teleporting = false; _siegeInterrupt = false; MODE:Release("siege"); break
            end
...
local function DungeonAttackLoop(onStatus)
    ...
    while DUNGEON.running and DUNGEON.inMap do
        ...
        PG_Wait(0.08) -- [PingGuard] Dungeon attack inner loop
    end
        ...
        if on then
            ...
            ANNIV.thread = task.spawn(function()
                ...
                while ANNIV.running do
                    ...
                    PG_Wait(0.5) -- [PingGuard] ANNIV step 4
                    ...
                    PG_Wait(1) -- [PingGuard] ANNIV step 7 (TP butuh waktu lebih)
                    ...
                    PG_Wait(0.5) -- [PingGuard] ANNIV step 8
                    ...
                    if not IsInAnnivMap() then
                        ...
                        if not tpOk or not ANNIV.running then
                            ...
                            while not IsInLobby() and lobbyWait < 6 and ANNIV.running do
                                PG_Wait(0.5) -- [PingGuard] ANNIV lobby confirm
                                lobbyWait = lobbyWait + 0.5
                            end
...
local function SetupUniversalSpy()
    ...
    -- Coba pasang __namecall hook (Delta/Xeno/Solara support)
    -- [FLa COMPAT] cek capability via FLa_CanHook() agar tidak crash di sUNC rendah
    local hookOk = false
    pcall(function()
...
do
 ...
 -- Helper folder — [FLa COMPAT] aman di semua executor
 local function _ensureFolder()
  -- makefolder & isfolder sudah di-polyfill di FLa COMPAT layer
 ...
 local function ListConfigs()
  _ensureFolder()
  -- listfiles sudah di-polyfill (return {} jika tidak support)
 ...
 local function SaveConfigAs(name)
  ...
  local ok, err = pcall(function()
   ...
   -- writefile sudah di-polyfill (warn jika tidak support)
 ...
 local function LoadConfigByName(name)
  local ok, result = pcall(function()
   ...
   -- isfile dan readfile sudah di-polyfill
 ...
 local function DeleteConfigByName(name)
  -- delfile sudah di-polyfill di FLa COMPAT (alias removefile atau overwrite kosong)
  local ok = pcall(function()
   local path = _cfgPath(name)
