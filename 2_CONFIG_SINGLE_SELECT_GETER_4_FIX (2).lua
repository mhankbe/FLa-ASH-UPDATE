...
do
    ...
    -- source: "RA" | "TA" | "MA"
    local function BlockAffectedTargetEffect(on, source)
        source = source or "MA"
        if on then
...
-- RA-STYLE thread: selalu menyerang guid HP TERTINGGI, re-rank tiap iterasi,
-- loop per-frame (task.wait() tanpa jeda besar) -- identik _MA_StartRAThread.
local function _SGMA_StartRAThread(getPullList)
    ...
    _SGMA_RA.thread = task.spawn(function()
        while _SGMA_RA.running and SIEGE.running and SIEGE.inMap do
            ...
            if #ranked == 0 then
                ...
                if _SGMA_RA.guid ~= best.guid then
                    _SGMA_RA.guid = best.guid
                end
                _SGMA_FireAtGuid(best.guid, best.model)
                task.wait()
            end
        end
        _SGMA_RA.running = false
...
-- TA-STYLE thread: sticky HP terendah, urut naik otomatis, pindah kalau ada
-- musuh baru dengan HP lebih rendah dari target sekarang -- identik _MA_StartTAThread.
local function _SGMA_StartTAThread(getPullList)
    if _SGMA_TA.running then return end
    _SGMA_TA.running = true
    _SGMA_TA.thread = task.spawn(function()
        while _SGMA_TA.running and SIEGE.running and SIEGE.inMap do
            local ranked = _SGMA_GetAliveRanked(getPullList())
            if #ranked == 0 then
                _SGMA_TA.guid = nil
                task.wait()
            else
                local lowest = ranked[1]
                for i = 2, #ranked do
                    if ranked[i].hp < lowest.hp then lowest = ranked[i] end
                end

                local curEntry = nil
                if _SGMA_TA.guid then
                    for _, e in ipairs(ranked) do
                        if e.guid == _SGMA_TA.guid then curEntry = e; break end
                    end
                end

                if not curEntry then
                    _SGMA_TA.guid = lowest.guid
                elseif lowest.guid ~= _SGMA_TA.guid and lowest.hp < curEntry.hp then
                    _SGMA_TA.guid = lowest.guid
                end

                local targetEntry = nil
                for _, e in ipairs(ranked) do
                    if e.guid == _SGMA_TA.guid then targetEntry = e; break end
                end
                if targetEntry then
                    _SGMA_FireAtGuid(targetEntry.guid, targetEntry.model)
                end
                task.wait()
            end
        end
...
local function SiegeAttackLoop(onStatus, d)
    ...
    -- Start 2 thread independen SEKALI di awal, sama seperti AttackLoop_Mass
    -- memanggil _MA_StartRAThread + _MA_StartTAThread sekali sebelum loop status.
    _SGMA_StartRAThread(getPullList)
    _SGMA_StartTAThread(getPullList)
